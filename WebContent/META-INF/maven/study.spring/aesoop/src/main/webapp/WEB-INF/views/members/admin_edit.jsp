...
<body>
    <h1>회원정보수정</h1>
    <form id="editForm" method="post" action="${pageContext.request.contextPath}/user/admin_edit_ok.do">
    	...
    	<div>
    		<label for="is_out">탈퇴 권한 여부: </label>
    		<label><input type="radio" name="is_out" id="is_out" value="Y" <c:if test="${output.is_out == 'Y'}">checked</c:if> />탈퇴회원</label>
    		<label><input type="radio" name="is_out" id="is_out" value="N" <c:if test="${output.is_out == 'N'}">checked</c:if> />일반회원</label>
    	</div>
    	
    	<div>
    		<label for="is_admin">관리자 권한 여부: </label>
    		<label><input type="radio" name="is_admin" id="is_admin" value="Y" <c:if test="${output.is_admin == 'Y'}">checked</c:if> />관리자</label>
    		<label><input type="radio" name="is_admin" id="is_admin" value="N" <c:if test="${output.is_admin == 'N'}">checked</c:if> />일반회원</label>
    	</div>
