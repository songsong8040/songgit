...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/assets/css/my_order_view.css" />
...
<body>
	...
	<div id="mypage_body">
		<div id="order_hd">
			...
			<h1>주문내역</h1>
			<span>주문한 상품의 배송상태를 확인할 수 있습니다. 언제나 빠른 발송, 배송이 가능하도록 최선을
				다하겠습니다.</span>
		</div>
		<br />
				...
				<tr class="tr_body">
					<c:forEach var="item" items="${output_prod}" varStatus="status">
						<tr height="70">
							<td>${item.prodno}</td>
							<td><a
								href="${pageContext.request.contextPath}/product_view?prodno=${item.prodno}">${item.pname}</a></td>
		...
		<table class="tprice_table">
			<tr>
				...
				<td class="td_con"><fmt:formatNumber pattern="###,###,###"
						value="${output.tprice}" />원</td>
			</tr>
		</table>
		...
		<!-- 주문취소 ----------------------------------------------------------------->
		<c:if
			test="${output.orderstatus == '결제완료' || output.orderstatus == '배송준비중'}">
			<div class="list_cancel">
				<form method="post"
					action="${pageContext.request.contextPath}/admin/order_list_cancel">
					<input type="hidden" name="orderno" value="${output.orderno}" /> <input
						type="submit" value="주문취소" class="btn btn-warning cancel_bt" />
				</form>
			</div>
		</c:if>
