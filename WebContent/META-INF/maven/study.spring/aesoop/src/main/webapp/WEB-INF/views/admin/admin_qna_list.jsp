...
<script type="text/javascript">
	...
	function deleteValue() {
		var url = "${pageContext.request.contextPath}/admin/qna_delete_ok.do"; // Controller로 보내고자 하는 URL (.dh부분은 자신이 설정한 값으로 변경해야됨)
			...
			if(chk == true) {
				$.ajax({
	                ...
	                success: function(json){
	                    if(json = 1) {
	                        alert("삭제가 완료되었습니다.");
	                        window.location = "${pageContext.request.contextPath}/admin/admin_qna_list.do";
	                    }
	                    else{
	                        alert("삭제를 실패하였습니다.");
	                    }
...
<body>
	...
	<div id="content" class="pull-left">
		<div id="content_pd">
			...
			<div class="title">질문과답변</div>
			<!-- 조건 검색 및 분류 -->
			<div class="search_box">
				<ul class="nav nav-pills" id="mytab">
					<li><a href="${pageContext.request.contextPath}/admin/admin_notice_list.do">공지사항</a></li>
					<li><a href="${pageContext.request.contextPath}/admin/admin_qna_list.do" class="active">Q&A</a></li>
				...
				<div class="c_search clearfix">
					...
					<form method="get"
						action="${pageContext.request.contextPath}/admin/admin_qna_list.do">

						<div class="sc_title pull-left">조건검색</div>

						<!-- 폼(1) - 분류 -->
						<div class="form-group pull-left">
							<select name="prod_view" id="prod_view"
								class="form-control pull-left">
								<option value="title"
									<c:if test="${qna_view == 'title'}">selected</c:if>>제목</option>
								<option value="username"
									<c:if test="${qna_view == 'username'}">selected</c:if>>작성자</option>
								<option value="content"
									<c:if test="${qna_view == 'content'}">selected</c:if>>내용</option>
							</select>
						</div>
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<c:forEach var="item" items="${output}" varStatus="status">
										...
										<%-- 상세페이지로 이동하기 위한 URL --%>
										<c:url value="/admin/admin_qna_view.do" var="viewUrl">
											<c:param name="qnano" value="${item.qnano}" />
										</c:url>

										<tr class="tr_body">
											<td>
												<div class="form-group">
													<input type="checkbox" name="RowCheck" id="RowCheck"
														class="form-control" value="${item.qnano}">
												</div>
											</td>
											<td align="center">${item.qnano}</td>
											<td align="center"><a href="${viewUrl}">${title}</a></td>
											<td align="center">${item.username}</td>
											<td align="center">${item.content}</td>
											<td align="center">
												<button type="submit" class="btn btn-warning" onclick="location.href='${pageContext.request.contextPath}/admin/admin_qna_edit.do?qnano=${item.qnano}'">수정</button>&nbsp;
												<button type="button" class="btn btn-danger" onclick="location.href='${pageContext.request.contextPath}/admin/admin_qna_delete_ok.do?qnano=${item.qnano}'" id="btn-delete">삭제</button> 
											</td>
										</tr>
									</c:forEach>
								</c:otherwise>
